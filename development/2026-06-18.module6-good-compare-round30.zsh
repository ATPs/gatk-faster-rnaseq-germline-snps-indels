...
GOOD_DIR=/XCLabServer002_fastIO/gatk-faster-rnaseq/SRR949115_broad_hg38_run2/module6_good_compare_20260618_round30
...
cargo build --manifest-path src/rust/Cargo.toml --release --bin rust_haplotype_caller --bin rust_hc_vcf_compare
...
bcftools norm -m -any -Oz -o "${GOOD_DIR}/java.raw.split.vcf.gz" "${JAVA_RAW}"
bcftools norm -m -any -Oz -o "${GOOD_DIR}/rust.raw.split.vcf.gz" "${RUST_RAW}"
bcftools index -f -t "${GOOD_DIR}/java.split.vcf.gz"
bcftools index -f -t "${GOOD_DIR}/rust.split.vcf.gz"
bcftools index -f -t "${GOOD_DIR}/java.raw.split.vcf.gz"
bcftools index -f -t "${GOOD_DIR}/rust.raw.split.vcf.gz"
...
./development/2026-06-17.good-compare-postprocess.zsh "${GOOD_DIR}" round30 20
